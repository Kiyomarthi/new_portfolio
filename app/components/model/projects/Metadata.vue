...
<template>
  ...
  >
    <div class="flex flex-wrap gap-6 text-h3">
      <div v-if="company" class="flex items-center gap-2 text-muted">
        <UIcon name="i-lucide-building" class="size-4" />
        <span class="font-medium text-highlighted">شرکت:</span>
        <span>{{ company }}</span>
      </div>

      <div v-if="startDate" class="flex items-center gap-2 text-muted">
        <UIcon name="i-lucide-calendar" class="size-4" />
        <span class="font-medium text-highlighted">شروع:</span>
        <span>{{ formatDate(startDate) }}</span>
      </div>

      <div v-if="endDate" class="flex items-center gap-2 text-muted">
